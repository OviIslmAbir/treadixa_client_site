...
import Bag from '../../Pages/BagsPage/Bag/bag';

const BagsSection = () => {
    const [data, setData] = useState([]);

    useEffect(() => {
    fetch('https://ecomerce-server-kohl.vercel.app/products?category=bags')
            .then(res => res.json())
            .then(actualData => {
                setData(actualData); 
            });
    ...
    return (
        ...
        <div className='container mx-auto py-4 px-1 md:px-4 bg-white rounded-xl md:rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-50 mb-5'>
            ...
            <div className="flex justify-between items-center mb-3 px-2">
                ...
                <Link to="/bags">
                    <button className="py-1.5 px-3 md:px-6 md:py-2 text-[10px] md:text-sm bg-[#7E53AC] border-none shadow-md rounded-full font-bold text-white">
                        VIEW ALL
                    </button>
                </Link>
            </div>
