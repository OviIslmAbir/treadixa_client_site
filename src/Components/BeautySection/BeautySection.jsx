...
import BeautyProduct from '../../Pages/BeautyPage/BeautyProduct/BeautyProduct';

const BeautySection = () => {
    const [data, setData] = useState([]);

    useEffect(() => {
        fetch('https://ecomerce-server-kohl.vercel.app/products?category=beauty')
            .then(res => res.json())
            .then(actualData => {
                setData(actualData); 
            });
    ...
    return (
        <div className='container mx-auto py-4 px-1 md:px-4 bg-white rounded-xl md:rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-50 mb-5'>
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-1.5 md:gap-4 px-1'>
                {
                    data.slice(0, 10).map(beautyProduct => (
                        <BeautyProduct 
